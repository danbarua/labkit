 ...
 * (`scripts/db/snapshot-record.ts`) that `bun run check` cannot assume exists,
 ...
 *
 * Usage:
 *   bun scripts/db/test-migration-safety.ts --snapshot <path.tar.gz> [--tenant labkit]
 */
...
import { age, pgliteAssets } from "@labkit/core-db/extensions.ts";
import { transactor } from "@labkit/core-db/transactor.ts";
import { resolveTenantContext } from "@labkit/core-db/tenant.ts";
import { scopeToTenant } from "@labkit/core-db/scoped.ts";
import { TenantGraph } from "@labkit/core-db/graph.ts";
import { connectDb } from "@labkit/core-db/connect.ts";
import { pgEventLog } from "@labkit/core-domain/event-store.ts";
import { NODE_LABELS, EDGE_LABELS } from "@labkit/core-db/domain.ts";
import { scalar } from "@labkit/core-db/cypher.ts";
import { bootstrapSession, type LabKitDB } from "@labkit/core-db/backend.ts";
