...
#   bash scripts/db/build-bonsai-record.sh [--deploy] [<db-dir>] [<gates-source-dir>]
#
...
probe() {
  ...
  shift
  echo "=== $name.sh"
  if ! bash "$root/scripts/db/$name.sh" "$@" > "$staging/build-$name.log" 2>&1; then
...
bash "$root/scripts/db/probe-bonsai-replay.sh" "$db" "$gates_source"
