 ...
 * so it lints itself. `scripts/checks/check-no-tracked-symlinks.sh` is the `.sh` one —
...
const EXEMPLAR = {
  ts: "scripts/checks/check-doc-comments.ts",
  sh: "scripts/checks/check-no-tracked-symlinks.sh",
} as const;
