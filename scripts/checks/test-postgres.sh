...
# `bash scripts/dev/compose.sh down`, stops it -- plain `docker compose down` works
...
root="$(git rev-parse --show-toplevel 2>/dev/null || (cd "$(dirname "${BASH_SOURCE[0]}")" && while [[ "$PWD" != "/" && ! -f "package.json" ]]; do cd ..; done && pwd))"
cd "$root"
...
eval "$("$root/scripts/dev/worktree-ports.sh" --export)"
url="${LABKIT_DB_URL:-postgres://postgres:agens@127.0.0.1:${LABKIT_PORT_DB}/$test_db}"

if [ -z "${LABKIT_DB_URL:-}" ]; then
  if ! bash "$root/scripts/dev/compose.sh" ps --status running --quiet db 2>/dev/null | grep -q .; then
    ...
    bash "$root/scripts/dev/compose.sh" up -d --build db
  fi

  echo -n "test:pg: waiting for postgres"
  for _ in $(seq 1 60); do
    if bash "$root/scripts/dev/compose.sh" exec -T db pg_isready -U postgres -q 2>/dev/null; then
      echo " — ready"
      break
    fi
    echo -n "."
  ...
  if ! bash "$root/scripts/dev/compose.sh" exec -T db pg_isready -U postgres -q 2>/dev/null; then
    echo
    echo "FAILED: postgres did not become ready. \`bash scripts/dev/compose.sh logs db\` has the reason."
    exit 1
  fi
else
...
if [ -n "${LABKIT_DB_URL:-}" ]; then
  ...
  after="The container is still up; \`bash scripts/dev/compose.sh down\` stops it."
fi
