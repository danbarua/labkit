 ...
 *
 * `scripts/checks/check-all.ts` did it first, as `argv: ["bun", "test"]`, and a build
 * failed at 5000ms with the flag apparently set. That was fixed; the next build
 * got further and failed the same way in `scripts/checks/test-postgres.sh`, the other
...
for (const file of readdirSync(DIR, { recursive: true }).map(String).sort()) {
  if (file.endsWith(SELF) || !(file.endsWith(".sh") || file.endsWith(".ts"))) continue;
