...
#
# Usage: bun run check:cli   (or: bash scripts/checks/smoke-cli.sh)
set -euo pipefail
...
lab is confirmed "$claim" --because 'the prespecified check passed at every depth' >/dev/null
lab close enquiry "$enquiry" --answered-by "$claim" >/dev/null
