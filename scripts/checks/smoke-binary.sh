...
refuse_db_url "${BASH_SOURCE[0]#"$root/"}"
db="$(mktemp -d "${TMPDIR:-/tmp}/labkit-binary.XXXXXX")"
trap 'rm -rf "$db"' EXIT

bash "$root/scripts/build/build-binary.sh" > /dev/null
...
# --- the record out as SQL and back, through the binary ---------------------
#
# `dump` runs pg_dump compiled to WASM, whose loader reads its own `.wasm` by a
# path that only exists inside the bundle when the build embeds it. A dump that
# restores into a second record and answers the same question is the only
# proof that it did.
...
echo "OK: the compiled binary migrates a fresh database, answers from it, dumps and restores it, and serves MCP."
