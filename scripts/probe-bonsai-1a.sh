...
q1=$(lab pose "does learned topology (T) produce distinguishable finite-time infinitesimal perturbation dynamics from matched controls (rewired, random, lattice)?")
...
loe2=$(lab pursue "$q2" --approach "10-class re-verification: 25 seeds per stochastic control, mean-aggregated paired Wilcoxon primary, median-aggregation + exact sign-flip + within-class MCSE robustness cascade, Holm correction across 4 comparisons")
task1=$(lab plan --objective "re-verify the T-vs-stochastic-control comparisons with 25 seeds per class, proper within-class aggregation and robustness checks" \
...
crit_hist=$(lab criterion "T vs historical half-edge random: $rule")
crit_curr=$(lab criterion "T vs current edge-count-matched random: $rule")
crit_rewire=$(lab criterion "T vs degree-preserving rewiring: $rule")
crit_lattice=$(lab criterion "T vs lattice: $rule")
...
lab evaluate "$crit_rewire"  --gate "$gate1" --value "primary p=0.01367 vs median p=0.19336" --outcome fail --citing "$clm5" >/dev/null
lab evaluate "$crit_lattice" --gate "$gate1" --value "primary, sign-flip, bootstrap all agree non-significant; no seed axis" --outcome pass --citing "$clm6" >/dev/null
...
out=$(lab replace "$comp3" --because "$rev1" --enquiry "$loe2" \
