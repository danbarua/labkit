...
async function fourComparisonsUnderOneRule() {
  ...
  const { criterion } = await session.writes.stateCriterion(RULE);
  ...
  for (const [name, proposition, outcome, value] of CONTROLS) {
    ...
    await session.writes.evaluateCriterion({ criterion, gate, about: claim, value, outcome });
...
describe("S-25: one rule judged four times", () => {
  ...
  test("the rule is stated once, not once per comparison", async () => {
    ...
    const found = await (await afterwards()).reads.search({ text: "noise floor" });
  ...
  test("one failing comparison blocks the gate, and the gate says which", async () => {
    ...
    // controls is four conditions on this gate, so the two that failed are two failed checks
    // naming which comparison — the answer to "which control did we not clear?", which is the
    // question a blocked gate raises.
