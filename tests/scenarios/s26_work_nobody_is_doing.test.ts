...
async function twoPlannedThings() {
  ...
  const { work: other } = await session.writes.planWork({
    objective: "profile the CPU sampler",
    acceptance: "a flame graph per stage",
  });
  const { criterion } = await session.writes.stateCriterion("GPU and CPU agree to 1e-6");
...
async function gatedPair() {
  const { work: stopped } = await session.writes.planWork({
    objective: "discarded GPU port",
    acceptance: "no longer needed",
  });
  const { work: active } = await session.writes.planWork({
    objective: "keep the CPU sampler",
    acceptance: "a flame graph per stage",
  });
  const { criterion } = await session.writes.stateCriterion("the sampler is numerically stable");
  const { gate } = await session.writes.declareGate({
    governedBy: [criterion],
...
describe("S-26: work nobody is doing", () => {
  test("Afterward 1: stopped work reads abandoned, and says why", async () => {
    ...
    const listed = await (await afterwards()).reads.workList({});
  ...
  test("Afterward 3: abandoned beats blocked — a gate no longer holds up work nobody is doing", async () => {
    ...
    const held = await (await afterwards()).reads.workList({});
    ...
    const dropped = await (await afterwards()).reads.workList({});
  ...
  test("work is stopped once: a second reason is refused, not recorded beside the first", async () => {
    ...
    await expect(
      session.writes.stopWork({ work, because: "actually the budget came back" }),
  ...
  test("stopped work is absent from blocked projections", async () => {
    ...
    const before = await (await afterwards()).reads.gateStatus({ gate });
    ...
    await session.writes.stopWork({ work: stopped, because: DROPPED });

    const after = await (await afterwards()).reads.gateStatus({ gate });
