...
describe("S-29: the note came after the question", () => {
  ...
  test("Afterward 2: `why` tells a note that prompted it from a note about it", async () => {
    const { question } = await session.writes.pose({ question: ASKS });
    const { note: because } = await session.writes.note({ text: PROBE, prompted: question });
    const { note: about } = await session.writes.note({
  ...
  test("Afterward 3: the same note can do both, and each edge keeps its own reading", async () => {
    const { question } = await session.writes.pose({ question: ASKS });
    const { note } = await session.writes.note({ text: PROBE, on: question, prompted: question });
  ...
  test("a second note prompting one question is refused", async () => {
    const { question } = await session.writes.pose({ question: ASKS });
    await session.writes.note({ text: PROBE, prompted: question });

    await expect(
      session.writes.note({ text: "another hunch entirely", prompted: question }),
