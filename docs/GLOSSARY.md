...
| **measured vs asserted** (a verdict) | An evaluation resting on cited evidence versus one citing nothing. Empty `basis` means asserted. `gate` prints the difference since #151; `evaluateCriterion`'s `citing` takes a claim, an observations handle, or an evidence handle directly, so a measured pipeline check with no scientific claim to cite is no longer forced to record as asserted | S-8, `Verdict.basis`, `EvaluateCriterionCommand.citing` |
...
| **a three-part refusal** | Every refusal names what was expected, what it got, and what would satisfy it, and may name a concept or a verb both surfaces spell the same — never a command | `ReadSurface` doc comment, `src/domain/read/index.ts`; #164, #169 |
| **partial supersession** | A re-analysis that revisits some of a prior analysis's conclusions and says nothing about the rest. `keep` names the ones that survive and supersedes the rest; `replace` is the whole-analysis case, which retracts all of them | `keep`, `replace`; #132, #173 |
| **`conclude`** | The primitive under `analyse`/`replace`/`reverify`: one conclusion per call, so a compound act is built by successive handle-carrying calls rather than described in a JSON blob | `labkit conclude`; #173 |
