 ...
 * `Addressing` in `packages/core-domain/report.ts`. Shared rather than inlined per
 ...
 * `packages/core-db/domain.ts`. Collected from the declarations, so a value added to an
