#!/usr/bin/env bash
#
# A research lifecycle, end to end, through the CLI and nothing else.
#
# **Every line here is a command a person could type.** That is the point, and
# it is what the TypeScript version this replaced could not be: it wrote by
# calling `TenantGraph.createNode` directly, underneath the domain layer, so it
# demonstrated that the persistence layer worked while demonstrating nothing
# about whether the research verbs were usable — and it put nodes on the record
# that no verb had recorded making.
#
...
# assertions on what came back — not on whether the commands ran. The previous
# version exited 99 on a completely successful run, so CLAUDE.md told everyone
# to ignore its exit code, and it then sat broken for 221 commits with no
# watcher. A signal nobody trusts has no watcher; that is the lesson, and it
# cost more than the script did.
#
...
#
# Usage: bash examples/full-lifecycle.sh
set -euo pipefail
